package com.sxdx.kiki.cloud.gateway.filter;

import com.alibaba.fastjson.JSONObject;
import com.sxdx.kiki.cloud.gateway.properties.KikiGatewayProperties;
...
@Slf4j
@Component
public class KikiGatewayRequestFilter implements GlobalFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        ServerHttpRequest build = request.mutate().header(KikiConstant.ZUUL_TOKEN_HEADER, new String(token)).build();
    ...
    private Mono<Void> checkForbidUri(ServerHttpRequest request, ServerHttpResponse response) {
        ...
        if (!shouldForward) {
            ...
            return makeResponse(response, kikiResponse);
        }
        return null;
    }

    private Mono<Void> makeResponse(ServerHttpResponse response, KikiResponse febsResponse) {
